...
base.setmetatable(_ui_env[_ui_env_ref_proxy], { __mode = "k", } )

--[[ the index listener for the item op ]]
local _ui_index = {}
base["_ui_index"] = _ui_index

--[[ script propertys , "name", "setter", "getter" ]]
--[[ { "text" = {d_setText, d_text}, "name" = {d_setName, d_name} } ]]
local _ui_script_propertys = {}
local _ui_p_setter = 1
local _ui_p_getter = 2
...
local _ui_proxy_mt = 
{
	__index = function( t, k)
		...
		-- access the script propertys
		local scriptPropertys = org[_ui_script_propertys]
		if not not scriptPropertys then
			local userPropertys = scriptPropertys[kstr]
			if not not userPropertys then
				local getter = userPropertys[_ui_p_getter]
				if not not getter then
					return getter(t)
				end	
			end
		end
	...
	__newindex = function( t, k, v)
		...
		if not not scriptPropertys then
			local userPropertys = scriptPropertys[kstr]
			if not not userPropertys then
				local setter = userPropertys[_ui_p_setter]
				if not not setter then
					return setter(t, v)
				end	
			end
		end
...
local function _ui_proxy_new_wrap(_control)
	local proxy = {}
	local args = {}
	base.setmetatable(args, { __mode="k",} )
	...
	-- need to register, the proxy
	if not not base.GUI then
		local _ui_lib = base.GUI
		_ui_lib.register(_control)
	end
	return proxy
end
...
local function _ui_proxy_assign(control, args)
	...
	-- call create event 
	local createEntry = args["onItemCreate"]
	if not not createEntry then
		createEntry(control, nil)
	end
...
function _ui_add_property(proxy, name, setter, getter)
	local org = base.rawget(proxy, _ui_index)
	local pps = org[_ui_script_propertys]
	if not pps then
		pps = {}
		base.rawset(org, _ui_script_propertys, pps)
	end

	pps[name] = {setter, getter}
end
