...
/*
  ...
  DYNAMIC_CRC_TABLE and MAKECRCH can be #defined to write out crc32_zip.h.
 */
...
#ifdef MAKECRCH
    /* write out CRC tables to crc32_zip.h */
    {
        FILE *out;

        out = fopen("crc32_zip.h", "w");
        if (out == NULL) return;
        fprintf(out, "/* crc32_zip.h -- tables for rapid CRC calculation\n");
