...
enum json_error 
json_array_insert_child (json_t * parent, json_t * child, json_t* next)
{
    /*TODO change the child list from FIFO to LIFO, in order to get rid of the child_end pointer */
    assert (parent != NULL);	/* the parent must exist */
    assert (child != NULL);	/* the child must exist */
    ...
    if (parent->child)
    {
        if (next)
        {
            ...
            child->next = next;
            //if next is the first child,next->previous is null,then child->previous will be none
            if (child->previous)
            {
...
json_t *dw_json_tree_find_array_helper(const json_t* object, const int index)
{
    ...
    cursor = (json_t*) object;
    //modify£¬if object is jsonarry,cursor can not be set to it child,or will wrong
    //if object is a <key,object> pair,need to set cursor to it is child
    if (cursor->type != JSON_OBJECT && cursor->type != JSON_ARRAY)
        cursor = cursor->child;
...
int dw_json_tree_count_array(const json_t *root, const char *label)
{
    ...
    if (!cursor->child)
        return 0;
    //modify:why need to set to child again...
    //cursor = cursor->child;
