...
void TiXmlElement::Print(const DwPtr<DwTextWriter>& textWriter, int depth) const
{
    DW_ASSERT(textWriter.isvalid());
	int i;
	for (i = 0; i < depth; i++)
	{
...
bool TiXmlDocument::LoadStream(const DwPtr<DwStream>& stream, TiXmlEncoding encoding)
{
	if (!stream.isvalid()) 
	{
...
bool TiXmlDocument::SaveStream(const DwPtr<DwStream>& stream) const
{
...
void TiXmlDocument::Print(const DwPtr<DwTextWriter>& textWriter, int depth) const
{
	const TiXmlNode* node;
	for ( node=FirstChild(); node; node=node->NextSibling() )
	{
...
void TiXmlAttribute::Print(const DwPtr<DwTextWriter>& textWriter, int /*depth*/ ) const
{
	TIXML_STRING n, v;
...
void TiXmlComment::Print(const DwPtr<DwTextWriter>& textWriter, int depth) const
{
...
void TiXmlText::Print(const DwPtr<DwTextWriter>& textWriter, int depth) const
{
	if (cdata)
	{
		int i;
...
void TiXmlDeclaration::Print(const DwPtr<DwTextWriter>& textWriter, int /*depth*/ ) const
{
...
void TiXmlUnknown::Print(const DwPtr<DwTextWriter>& textWriter, int depth ) const
{
	for ( int i=0; i<depth; i++ )
		textWriter->writeString("    ");
	textWriter->writeFormatted("<%s>", value.c_str());
